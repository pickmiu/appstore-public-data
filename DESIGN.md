...
2. **App Store 纯应用榜单快照**：每日北京时间 00:15（UTC 16:15），生成中美两区（CN/US）的纯应用主榜单（Top 100，自动剔除所有游戏）、纯应用一级品类（Top 10）的 Markdown 状态快照，彻底排除所有游戏相关内容。
...
### 5.1 时间与触发
* **触发时间**：每天北京时间 00:15 错峰触发。对应 GitHub Actions 的 UTC 时间为前一天的 **16:15**（Cron: `15 16 * * *`）。
